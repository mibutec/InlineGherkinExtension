...
public class DefaultPojoMapper<T> implements PojoMapper<T> {
			...
			for (Entry<String, String> entry : map.entrySet()) {
				if (!setFieldBySetter(ret, entry.getKey(), entry.getValue())) {
					if (!setField(ret, entry.getKey(), entry.getValue())) {
						throw new IllegalStateException("couldn't find any field or setter named " + entry.getKey() + " in " + targetType.getSimpleName());
					}
				}
			}
	...
	protected boolean setFieldBySetter(T target, String name, String value) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		for (Method m : target.getClass().getMethods()) {
			if ((m.getName().equalsIgnoreCase(name) || m.getName().equalsIgnoreCase("set" + name)) && m.getParameterTypes().length == 1) {
	...
	protected boolean setField(T target, String name, String value) throws IllegalArgumentException, IllegalAccessException {
